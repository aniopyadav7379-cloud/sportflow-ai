...
export default function RegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const user = await apiRequest("/api/auth/register", "POST", { name, email, password });
      await mutate(user, { revalidate: false });
      toast.success(`Welcome to SportFlow AI, ${user.name}!`);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-[#0B1330] via-[#141c42] to-[#1c2a5e] p-4">
      ...
      >
        ...
        <h1 className="mb-1 text-[19px] font-semibold text-slate-900 dark:text-white">Create your account</h1>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div>
            <label className="mb-1 block text-[12px] font-medium text-slate-600 dark:text-slate-300">Full Name</label>
            <input required value={name} onChange={(e) => setName(e.target.value)} className="input" placeholder="Anoop Yadav" />
          </div>
          <div>
